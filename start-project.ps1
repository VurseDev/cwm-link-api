...
Ensure-Tool 'node' 'Install Node.js LTS from https://nodejs.org and reopen PowerShell.'
Ensure-Tool 'npm' 'Install Node.js LTS from https://nodejs.org and reopen PowerShell.'
...
if (-not $SkipInstall) {
    ...
    if (-not (Test-Path (Join-Path $backendDir 'node_modules'))) {
        Invoke-CommandIn $backendDir 'npm' @('install')
    }

    Write-Step 'Installing frontend dependencies'
    if (-not (Test-Path (Join-Path $clientDir 'node_modules'))) {
        Invoke-CommandIn $clientDir 'npm' @('install')
    }
...
Invoke-CommandIn $backendDir 'npm' @('run', 'prisma:generate')
Invoke-CommandIn $backendDir 'npm' @('run', 'prisma:deploy')
...
$apiCommand = 'cd /d "{0}" && set PORT={1} && npm run start:dev' -f $backendDir, $chosenPort
$frontendCommand = 'cd /d "{0}" && npm run dev' -f $clientDir
